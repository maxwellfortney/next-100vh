      ...
      <div className="flex flex-col flex-1 w-11/12">
        <div className="flex justify-between w-full">
            ...
            {session && session.user?.image ? (
              <>
                <img
                  ...
                  alt={session.user?.name ? session.user?.name : "A 100vh user"}
                />
          ...
          <div className="flex flex-col items-center self-end justify-between flex-none text-white">
            <a onClick={() => signOut()} className="flex mb-10">
              logout
            </a>
            <div className="flex text-xl font-extrabold ">
              <Button100VH styleType={2} label="follow" className="mr-2" />
              <Button100VH styleType={3} label="message" />
            </div>
          </div>
        </div>
