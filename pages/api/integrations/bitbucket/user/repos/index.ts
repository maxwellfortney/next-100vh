...
export default async function handler(
    ...
    if (session) {
        const bitbucketRes = await fetch(
            `https://api.bitbucket.org/2.0/repositories?access_token=${session.accessToken}&role=admin`
        );

        console.log("status", bitbucketRes.status);

        if (bitbucketRes.status === 200) {
            const reposRes = await bitbucketRes.json();
            console.log(reposRes);
            res.status(200).json(JSON.stringify(reposRes.values, null, 2));
            return;
        } else {
            res.status(404).json(
