...
export default function AComment({ comment }) {
  ...
  async function handleToggleLike(e) {
    ...
    if (didFetchIsLiked) {
      ...
      const res = await fetch(
        `/api/users/${username}/projects/${title}/comments/${comment._id}/likes`,
        {
  ...
  useEffect(() => {
    if (session && !loading && !didFetchIsLiked) {
      fetchDidLike();
    }
  }, [session, loading, didFetchIsLiked]);
    ...
    >
      <div className="flex items-center justify-between">
        <div className="flex items-center">
          ...
          />
          <p className="font-semibold">{comment.commentOwnerUsername}</p>
        </div>
        <button
