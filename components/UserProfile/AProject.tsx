...
export default function AProject({
          ...
          <html>
            ...
            <body>
                <div class="overflow-hidden">
                    <div id="Project-100vh-Container" class="absolute" style="transform: scale(0.5, 0.5); width: 200%; height: 200%; left: -50%; top: -50%">
                        ${html || ""}
                    </div>
                </div>            
            </body>
  ...
  useEffect(() => {
    ...
    const iframe = document.querySelector(
      `#${ownerUsername}-Project-${i}`
    ) as HTMLIFrameElement;
    (iframe as any).src = url;

    iframe.onload = () => {
      const doc = iframe.contentWindow?.document.querySelector("#testDiv");
        ...
        >
          <div className="flex items-center justify-between w-11/12 text-2xl text-white">
            ...
            >
              ...
              <p className="ml-2 text-sm font-normal">{likesState}</p>
            </div>
          </div>
        </div>
        <iframe
